#!/usr/bin/env python3
"""
Work Queue-Based GPU Parallelized Steering Script

This script performs activation steering using a work queue for dynamic load balancing.
Work units are distributed via a shared queue, allowing GPUs to automatically balance
load by pulling work as they become available.

Features dynamic batch processing, perfect load balancing, and advanced restart support.
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="Work queue-based GPU parallelized steering script",
...
def worker_process(
    ...
    """
    Worker process that pulls work batches from queue and processes them.
    """
    torch.set_float32_matmul_precision('high')
...
def main():
    """Main function to orchestrate work queue-based GPU steering."""
    args = parse_arguments()
    
    print("="*60)
    print("Work Queue-Based GPU Parallelized Steering")
    print("="*60)
