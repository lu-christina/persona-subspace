...
COEFFICIENTS = df["magnitude"].tolist()
    ...
    if do_steering:
        ...
        for magnitude in magnitudes:
            ...
            if magnitude == 0.0:
                ...
                try:
                    if len(COEFFICIENTS) == len(feature_directions):
                        magnitude_coeffs = [magnitude * COEFFICIENTS[i] for i in range(len(feature_directions))]
                    else:
                        magnitude_coeffs = [magnitude] * len(feature_directions)
                        
                    with ActivationSteering(
                        model=model,
                        steering_vectors=feature_directions,
                        coefficients=magnitude_coeffs,
                        layer_indices=STEERING_LAYER,
